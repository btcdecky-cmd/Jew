...
const products = [
  { name: "The Aurelia", category: "Branded Watches", price: "$980", image: "https://images.pexels.com/photos/38946825/pexels-photo-38946825.jpeg?auto=compress&cs=tinysrgb&w=900" },
    ...
    <main className="min-h-screen overflow-hidden bg-obsidian text-ivory">
      ...
      <header className="absolute left-0 right-0 top-8 z-30 border-b border-ivory/20 bg-obsidian/10 backdrop-blur-md">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-5 lg:px-12">
          <button onClick={() => navTo("top")} className="flex items-center gap-3" aria-label="Aurelia Maison home">
            <span className="flex h-9 w-9 items-center justify-center border border-champagne font-serif text-2xl text-champagne">A</span>
            <span className="text-left"><strong className="block text-[11px] uppercase tracking-[0.3em]">Aurelia Maison</strong><small className="mt-1 block text-[8px] uppercase tracking-[0.24em] text-ivory/55">Curated essentials</small></span>
          </button>
      ...
      <section id="top" className="relative flex min-h-[700px] items-end overflow-hidden pb-16 pt-40 lg:min-h-[850px] lg:pb-24">
        <img src={heroImage} alt="Woman wearing statement jewelry from the Aurelia Maison collection" className="absolute inset-0 h-full w-full object-cover object-center" />
      ...
      <footer className="border-t border-ivory/10 bg-obsidian-soft px-5 py-12 lg:px-12 lg:py-16"><div className="mx-auto max-w-[1440px]"><div className="grid gap-10 md:grid-cols-[1.4fr_1fr_1fr_1fr]"><div><span className="font-serif text-3xl text-champagne">Aurelia Maison</span><p className="mt-5 max-w-xs text-sm leading-6 text-ivory/50">Luxury essentials for women who define their own style.</p></div><FooterColumn title="Shop" items={["Collections", "New arrivals", "Best sellers", "Sale"]} /><FooterColumn title="Client care" items={["Shipping & returns", "Care guide", "Contact us", "Book an appointment"]} /><div><p className="mb-4 text-[10px] font-semibold uppercase tracking-[.22em] text-ivory/45">Follow the maison</p><div className="flex gap-5 text-sm text-ivory/65"><button className="hover:text-champagne">Instagram</button><button className="hover:text-champagne">Pinterest</button></div></div></div><div className="mt-14 flex flex-col justify-between gap-4 border-t border-ivory/10 pt-5 text-[9px] uppercase tracking-[.2em] text-ivory/35 sm:flex-row"><span>© 2026 Aurelia Maison</span><span>New York · London · Everywhere</span></div></div></footer>
    </main>
  );
}
